...
exports.createProfile = async (userId, reqBody, session) => {
  try {
    ...
    const profile = new Profile(extractedData);
    ...
    try {
      ...
      // If the data is valid, the document will be saved,
      // and you can send a success response
      res.json({ message: 'Profile saved successfully' });
    } catch (error) {
      // If the data is not valid, Mongoose will throw a validation error,
      // and you can send an error response
      res.status(400).json({ message: 'Invalid profile data', error: error.message });
    }

    // 5. Return the created profile document
    return profile;
  } catch (error) {
    console.error(error);
...
function extractProfileData(reqBody) {
  ...
  return extractedData; // Return the extracted data object
}

// Function to update a profile
exports.updateProfile = async (userId, profileId, reqBody, session) => {
  try {
    // 1. Extract profile data from request body
    const extractedData = extractProfileData(reqBody); // Call extractProfileData function
